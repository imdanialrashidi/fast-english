...
Use this as a focused evidence pass, not as a replacement for Pi's browser, design, review, or test tooling. Keep the scope to the changed
...
2. Inspect semantics and interaction with the Playwright MCP browser (snapshot, focused find, keyboard interaction, focus state, console/network evidence). Use
   the repository's existing browser test framework when it already exists; do
   not install axe or a second accessibility framework merely
   for this audit.
