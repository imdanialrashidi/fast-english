...
OMP runs with the operating-system permissions of the account that starts it. Project trust, prompts, tool allowlists, and `.omp/extensions/safety-guard.js` are defense in depth; they are not a sandbox and cannot reliably parse every shell/interpreter behavior or redirect. This matches OMP's official security model: project trust controls repository-provided input, while real isolation must come from the operating system, container, or VM.

For a trusted working copy, `omp` is autonomous by default: it passes `--approve`, allows routine workspace/Git/browser execution, and does not insert confirmation popups. The guard still blocks high-blast-radius patterns such as credential-store access, destructive deletion, force/deleting pushes, publication/deployment, production mutation, and browser file upload. `OMP_PROJECT_TRUST=ask|never` and `OMP_GUARD_MODE=strict` remain explicit diagnostic/restriction overrides.
...
2. Use `scripts/omp-sandbox.sh` for a simple local Docker boundary, or follow OMP's official containerization guidance for alternatives.
3. Pass only the provider keys required for the task. The wrapper intentionally does not mount the host OMP home, SSH keys, cloud credentials, or Docker socket.
4. Review the repository, `.omp/config.yml`, packages, extensions, skills, prompts, and MCP configuration before trusting them.
...
OMP packages and extensions execute with the OMP process's permissions. Versions in `.omp/config.yml` are exact pins, but a pin is not a source review. Before updating:
...
- run `node scripts/validate-workflow.mjs` against the OMP template;
- rerun `bash scripts/omp-doctor.sh --ci --static`.
