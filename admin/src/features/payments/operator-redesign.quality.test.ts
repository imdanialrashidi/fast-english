// admin/src/features/payments/operator-redesign.quality.test.ts
...
function read(rel: string): string {
  return readFileSync(resolve(ROOT, 'admin', 'src', rel), 'utf8');
}

const ITEM = 'features/payments/components/OperatorRequestItem.tsx';
const CHIP = 'features/payments/components/OperatorStatusChip.tsx';
const QUEUE = 'features/payments/components/OperatorQueue.tsx';
const EMPTY = 'features/payments/components/OperatorEmptyState.tsx';
const DETAIL = 'features/payments/components/OperatorRequestDetail.tsx';
const WORKSPACE = 'features/payments/components/OperatorWorkspace.tsx';
const APPROVE = 'features/payments/components/ApproveRequestDialog.tsx';
const REJECT = 'features/payments/components/RejectRequestDialog.tsx';
const RECEIPT = 'features/payments/components/OperatorReceiptInspector.tsx';
const PANEL = 'features/payments/components/OperatorDecisionPanel.tsx';
const TOOLBAR = 'features/payments/components/OperatorQueueToolbar.tsx';
const STALE = 'features/payments/components/OperatorStaleState.tsx';
...
describe('Operator detail hierarchy', () => {
  ...
  it('requestId support codes appear only inside safe error details', () => {
    ...
    const errors = read('features/payments/errors.ts');
...
describe('Operator decisions', () => {
  ...
  it('public reason and internal note stay visually + contractually separated', () => {
    ...
    expect(reject).toContain('فقط برای مدیریت');
    const api = read('features/payments/api.ts');
...
describe('Workspace geometry and shell', () => {
  ...
  it('selection preserves the queue filter query in the URL', () => {
    ...
    expect(workspace).toContain('navigate(`/payments/');
    expect(workspace).toMatch(/payments\/\$\{id\}/);
  });
  ...
  it('admin shell offers Logout and no Student navigation', () => {
    // The Admin Console has its own shell; the Student app header must
    // contain neither Staff controls nor a theme toggle (Podcast Slice 1).
    const header = readFileSync(
      resolve(ROOT, 'app', 'src', 'app', 'shell', 'AppHeader.tsx'),
      'utf8',
    );
    expect(header).not.toContain('ThemeSwitch');
    expect(header).not.toContain('LogoutRoundedIcon');
    expect(header).not.toContain('پنل اپراتور');
    const adminShell = readFileSync(
      resolve(ROOT, 'admin', 'src', 'shell', 'AdminShell.tsx'),
      'utf8',
    );
    expect(adminShell).toContain('admin-logout');
    expect(adminShell).toContain('LogoutRoundedIcon');
  });
});
