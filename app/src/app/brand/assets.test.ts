...
import { logoPaths } from '../brand/Brand';

const ROOT = process.cwd();
const SOURCE_DIR = join(ROOT, 'fast_english_logo_assets');
const APP_BRAND_DIR = join(ROOT, 'app', 'src', 'assets', 'brand');
...
describe('brand assets', () => {
  ...
  it('SVG mark is a monochrome currentColor treatment of the approved geometry', () => {
    ...
    const markSource = readFileSync(join(ROOT, 'app', 'src', 'app', 'brand', 'Brand.tsx'), 'utf8');
  ...
  it('renders the header PNG only on Light surfaces (black artwork check)', () => {
    ...
    const brandSource = readFileSync(join(ROOT, 'app', 'src', 'app', 'brand', 'Brand.tsx'), 'utf8');
