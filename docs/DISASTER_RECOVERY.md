# Fast English Podcast — Disaster Recovery (full VPS loss, self-hosted)

> Self-hosted Coolify era. Purpose: restore the service on a BRAND-NEW VPS after total
> loss of the production server. Also the "what must exist outside the failed
> VPS" inventory. **Last updated:** 2026-08-28.
...
| Production images | GHCR (`ghcr.io/<owner>/fast-english/*:sha-<commit>` — immutable; `production` is only a secondary alias) | published per release |
...
| Credential custody | Operator vault: self-hosted Coolify dashboard login + API token (`COOLIFY_BASE_URL` + `COOLIFY_API_TOKEN`), GitHub admin, DNS registrar, VPS provider console, Android keystore + passwords | human-managed |
...
| Coolify access | Self-hosted Coolify instance (reinstalled from its official installer; apps recreated from `docs/COOLIFY_DEPLOYMENT.md` §6; data survives on VPS + off-VPS backups) | human-managed |
...
(the Coolify apps are recreated from `docs/COOLIFY_DEPLOYMENT.md` §6) and the
self-hosted Coolify instance is reinstallable.

## 1. Recovery procedure (new VPS)

```text
1.  Order a new VPS (same provider or any); record its IP.
2.  Provider firewall: 22 (restricted to operator IP) + 80/443.
...
4.  Install self-hosted Coolify on the new VPS (official installer).
    Create the API token and record the base URL (COOLIFY_BASE_URL).
...
6.  Deploy the exact known-good images: recreate the four Coolify
    Applications (COOLIFY_DEPLOYMENT.md §6) pinned to the LAST GREEN
    sha-<commit> (from the last release summary or GHCR tags) via
    PATCH sha-<commit> + deploy --force.
...
9.  Frontends: start the Landing/Student/Admin apps (same pinned sha-<commit> images).
...
| Only the VPS disk | Same as §1 but DNS/TLS may survive; reinstall Coolify + re-provision + restore |
| Self-hosted Coolify instance lost (but VPS survives) | Reinstall Coolify (official installer), recreate the four apps from §6 of COOLIFY_DEPLOYMENT.md pinned to the last green `sha-<commit>`; app data untouched (it lives on the VPS + backups at `/opt/fast-english/shared/pb_data` and S3) |
