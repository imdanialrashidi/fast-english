...
- `pnpm verify:full` — the canonical full application gate: `scripts/project-verify.sh` (fast + all 14 real-PB smoke suites + deterministic three-surface builds + topology/boundary checks + PWA output checks + Android version/signing fail-safe + deploy redaction proofs) followed by the full Playwright suite. `scripts/verify.sh` is the compatibility entry that delegates here for CI and release tooling; `scripts/verify-full.sh` is the executable gate.
