...
- `pnpm verify:full` — the canonical full application gate: `scripts/project-verify.sh` (fast + all 18 real-PB smoke suites + deterministic three-surface builds + topology/boundary checks + PWA output checks + Android version/signing fail-safe + deploy redaction proofs) followed by the full Playwright suite. `scripts/verify.sh` is the compatibility entry that delegates here for CI and release tooling; `scripts/verify-full.sh` is the executable gate.
- CI (`.github/workflows/quality.yml`) runs the same contracts as independent lanes: `static` (harness doctor + fast verification), `backend` (`scripts/project-verify.sh` with `FEP_VERIFY_PARALLEL_SMOKES=1`, so the 18 smoke suites run concurrently on their own disposable PocketBases via `scripts/verify-smokes-parallel.sh`), and `e2e` (the full Playwright suite sharded `1/5..5/5` across runners, each shard with its own disposable PocketBase and built preview servers). The local `pnpm verify:full` serial model is unchanged.
...
Confirmed from `package.json`, `vite.*.config.ts`, `scripts/*`, `server/pb_hooks/*`, and CI:

- **Install:** `bash scripts/ci-install.sh` (CI) or `pnpm install --frozen-lockfile` + `pnpm setup:pocketbase` (downloads `server/pocketbase` pinned by `server/VERSION` = 0.39.9). Requires `corepack enable`, Node `>=24 <25` (`.nvmrc` = 24), pnpm `>=11 <12` (`packageManager` = 11.17.0). Single `pnpm-lock.yaml` v9; no workspace framework.
- **Local start (persistent dev):** `bash scripts/dev.sh` → PocketBase `127.0.0.1:8090` against persistent `server/pb_data/` (survives restarts; `PB_DEV_EPHEMERAL=1` opts into disposable `/tmp`); `pnpm dev:app` → Vite `http://127.0.0.1:5173` (`vite.app.config.ts` + `vite.base.ts` `apiProxy` → `VITE_API_TARGET` default `http://127.0.0.1:8090`); `pnpm dev:landing` → `127.0.0.1:5174`; `pnpm dev:admin` → `127.0.0.1:5175`. Production builds: `pnpm build:landing` (→ `dist-landing/` + `node scripts/prerender-landing.mjs`), `pnpm build:app` (→ `dist-app/`), `pnpm build:admin` (→ `dist-admin/`), `pnpm build` (all three). Capacitor `webDir=dist-app`, `android/` committed.
- **Health / readiness signals:** `GET http://127.0.0.1:8090/api/health` (real PB; `{"code":200}`), `GET /api/fast-english/public/settings` (public pricing/support), `GET /api/fast-english/library` (authenticated), `#root[data-app-version][data-build-time]` injected by `vite.base.ts` `transformIndexHtml` (also `__APP_VERSION__`/`__BUILD_TIME__` defines). `deploy/ops-check.sh` aggregates container + public HTTPS + cert expiry + disk + backup freshness. Browser health: `window.__fepTelemetry()` ring-buffer (200 entries) + `window.__fepNativeBridge` guard.
- **Relevant logs / diagnostics:** Caddy access logs `/opt/fast-english/shared/logs/access-*.log` (rotated 10×10 MiB, 30 d; `request>uri` filtered `token=[REDACTED]` verified by `deploy/test-log-redaction.sh`; `log_credentials` never enabled), PocketBase `journalctl -u fast-english-pocketbase` (retention `logs.maxDays=30`, `logAuthId=false`), Vite dev logs `/tmp/fep-vite-*.log`, Playwright traces `test-results/e2e/`. Failure evidence: concise `vitest`/`biome`/`tsc` output, `scripts/smoke-*.mjs` TAP, browser console/network via `mcp` snapshot + `browser_evaluate`, health endpoint JSON.
- **Test entrypoints (Vitest = pure logic/static guards; Smoke = real PB contract; Playwright = browser journeys only):** `pnpm test` / `pnpm test:coverage` (`vitest.config.ts` includes `app/**/*.test.ts`, `shared/**/*.test.ts`, `admin/**/*.test.ts`, `landing/**/*.test.ts`), `pnpm smoke:auth|payment|payment-preview|placement|placement-levels|placement-race|placement-capacity|lessons|episode|progress|podcast-domain|content-import|content-admin|library|business-settings|staff|operator` (each `bash scripts/smoke-*.sh node scripts/smoke-*.mjs` on disposable `/tmp/pb-*`), `pnpm test:e2e:fast -- e2e/<spec>.spec.ts` (low-resource lane), `pnpm test:e2e:full` (CI lane). `scripts/verify-smokes-parallel.sh` runs 18 smokes concurrently for CI (`FEP_VERIFY_PARALLEL_SMOKES=1`).
- **Browser entrypoints:** `playwright.config.ts` — `PW_FAST=1` via `scripts/playwright-fast.sh` (one Chromium `Desktop Chrome`, 1 worker, 0 retries, `maxFailures=1`, `list` reporter, `video:off`, `trace:off`, `screenshot:off`, `reuseExistingServer:true`, `locale:fa-IR`); CI uses built `vite preview` (5173/4174/4175) + disposable PB `PB_E2E_PORT=18101` + `global-setup.ts`/`global-teardown.ts` + sharded `1/5..5/5`. Deterministic specs: `e2e/**.spec.ts`; interactive MCP exploration uses `mcp` proxy separately and is NOT a substitute for committed specs.
- **Database / test-state setup:** `server/pb_migrations/*.js` (30 migrations, JS committed) + `server/pb_hooks/*.pb.js` (18 hook files, Goja ES5) + `server/VERSION`; `server/pb_data/` git-ignored; smoke/e2e wrappers create throwaway `/tmp/pb-*` data dirs + ephemeral superuser via `scripts/pb-test-helper.sh pb_create_superuser` (credentials never logged); `scripts/seed/plans.mjs` + `seeds/business/plans.json`; `pnpm content:validate|plan|import` for `schemas/episode-package.schema.json` (`additionalProperties:false`).
- **Rollback / recovery mechanism:** Immutable releases `/opt/fast-english/releases/<id>` + atomic `current` symlink (`deploy/deploy.sh` pre-backup + health + smoke + automatic symlink rollback on failure; previous release never deleted; `pb_data` outside releases). DB: `pb_backups` (`backups.cron` 02:30 UTC keep 14 + `fast-english-backup-copy.timer` 02:40 off-`pb_data` + `deploy/restore-drill.sh` on disposable instance). APK: immutable `releases/fast-english-podcast-v*.apk` + `verify-release-apk.sh` (apksigner/zipalign/sha256 → `release-metadata.json`). Caddy `systemctl reload`, PB `systemctl restart fast-english-pocketbase`.

### Regression test defect sensitivity

Every new regression test must demonstrate it would have failed without the fix. Preferred proof (in order): (1) pre-fix/red run — stash the fix, run the single test, show the assertion failure, then restore; (2) mutation/boundary — flip the corrected condition/value (e.g., re-introduce the off-by-one, missing authz check, or leaked field) and show the test fails; (3) equivalent oracle — compare against the authoritative source (real PB API, snapshot fixture, or smoke assertion) rather than the implementation's own output. For `test-design` changes, include the distinct failure model, the independent oracle, the cheapest faithful layer, and the defect-sensitivity evidence in the PR body (`--body-file .artifacts/ai-pr.md`). Do not add coverage-only or redundant tests that cannot exhibit a pre-fix failure.

### Flaky test isolation (no normalized retries)

An intermittent failure is a reliability defect — never hide it with retries. Locally: `PW_FAST=1` uses `retries:0`; CI allows at most one retry only in the full lane and records `trace:on-first-retry` + `screenshot:only-on-failure` under `test-results/e2e/` per shard. When a test flakes: (a) quarantine with `test.fixme` or `test.skip` + linked issue and keep the original failure log, (b) isolate the suite with `pnpm test:e2e:failed` (`--last-failed`) or `node --test --test-only`, (c) fix the root cause (race, missing await, shared `/tmp` state, or real entitlement timing) before re-enabling, and (d) keep the quarantine bounded (remove skip within one slice). The smoke harness awaits all scenarios before summarizing so failures affect the exit code; never add `retry:2` to Vitest unit tests.
