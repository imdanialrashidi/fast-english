...
- CI (`.github/workflows/quality.yml`) runs the same contracts as independent lanes: `static` (harness doctor + fast verification), `backend` (`scripts/project-verify.sh` with `FEP_VERIFY_PARALLEL_SMOKES=1`, so the 16 smoke suites run concurrently on their own disposable PocketBases via `scripts/verify-smokes-parallel.sh`), and `e2e` (the full Playwright suite sharded `1/5..5/5` across runners, each shard with its own disposable PocketBase and built preview servers). The local `pnpm verify:full` serial model is unchanged.
