...
| A7 — existing behavior + server entitlement boundaries unchanged | PARTIAL (gate blocked by pre-existing full-suite debt) | no server code changed; nonce provably inert (proxy reads only `token`); podcast-domain SMOKE suite green in `verify:full` project gate; ALL 16 smoke suites + builds + topology + PWA + Android checks green | full Playwright suite green on the built app — BLOCKED by pre-existing non-idempotent beforeAll seeding in accepted prior-slice specs (16× fixture duplication per run — sqlite evidence: 16 «دسته آلفا» categories) + their rate-window interactions |
...
5. ~~`pnpm verify:full`~~ EXECUTED — project gate (verify:fast + all 16 smoke suites + builds + topology + PWA + Android + deploy redaction proofs) GREEN; full Playwright suite on the built app FAILS on PRE-EXISTING full-suite debt (non-idempotent beforeAll seeding in accepted prior-slice specs → fixture multiplication + rate-window 429s; sqlite evidence). Slice-8 tests pass in the full lane except the T7 float boundary (fixed) and library-11 pagination derivation (fixed).
6. Delivery: task branch `slice8-player-lifecycle-reliability`, commit the scoped tree, push, PR. Never touch `main` directly. Follow-up workstream: make the prior-slice e2e beforeAlls idempotent (owned-fixture dedupe), then re-run `pnpm verify:full` to close A7's full-suite e2e evidence.
...
- **FULL-GATE BLOCKER (pre-existing, not slice 8):** the canonical Playwright suite on the built app fails because the accepted prior-slice specs' `beforeAll` seeding is non-idempotent — the library/podcast-episode/podcast-domain `beforeAll`s re-run multiple times in one run, multiplying every fixture (sqlite evidence from a single run: 16 «دسته آلفا» + 16 «دسته بتا» + 16 «دسته پیشنویس» categories; episode counts 62+). Consequences: strict-mode locator collisions, hard-coded counts, pagination and geometry assertions breaking, and progress-rate-window 429s under the multiplied request volume. These specs were accepted but never verified in full-suite composition; fixing them is a separate workstream (owned-fixture dedupe like `ensureOwnedPlan` in e2e/fixtures.ts, per-spec).
