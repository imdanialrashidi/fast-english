...
async function seedFixtures(su: string) {
  ...
  // A2 lesson — realistic Persian title (the Episode surface H1 is the
  // Persian title; the English title is the LTR caption).
  const a2Topic = await makeTopic(su, {
    slug: `t-a2-${randId()}`,
    status: 'published',
    sort_order: 3,
    title_fa: 'گفت‌وگو در سطح A2',
  });
...
test.describe('P3-S1 Lessons E2E', () => {
  ...
  // ------------------------------------------------------------------
  test('eligible student sees lessons list with correct lessons', {
    tag: ['@critical', '@smoke'],
  }, async ({ page }) => {
  ...
  test('student can open a lesson at another published level', { tag: '@critical' }, async ({
    ...
    // Cross-level access now works: the A2 Episode renders (Persian H1).
    await expect(page.getByRole('heading', { name: 'گفت‌وگو در سطح A2' }).first()).toBeVisible({
      timeout: 10_000,
    });
  });
