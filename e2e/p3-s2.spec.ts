...
test.describe('P3-S2 Progress and Audio Player', () => {
  ...
  test('2 - opens a real lesson', async ({ page }) => {
    ...
    // The Episode surface H1 is the Persian title (Slice 7).
    await expect(page.getByRole('heading', { name: 'اپیزود اول آزمایشی' }).first()).toBeVisible({
      timeout: 10_000,
    });
  });
  ...
  test('16 - resume restores saved position in the player', async ({ page }) => {
    ...
    // The player seeks to the saved position and starts playback. The
    // fixture file is 2s long, so the applied seek clamps to its end — the
    // slider proves the saved position (150s) was actually sought, not
    // skipped.
    const player = page.locator('[role="group"][aria-label="پخش‌کنندهٔ صوت"]');
  ...
  for (const viewport of VISUAL_VIEWPORTS) {
    test(`[${viewport.name}] lesson list and audio player are reachable`, async ({ page }) => {
      ...
      // Slice 7 deck contract: the primary control is the labeled CTA in
      // the fixed 56px slot (the fixture lesson is in-progress here, so the
      // label is the resume CTA — assert the slot, not a specific label).
      ...
      // The speed menu opens without an entrance transform, so the items
      // measure at their final 44px targets immediately.
      await expect
        .poll(async () => {
          ...
          return sizes.every((s) => s.width >= 44 && s.height >= 44);
        })
        .toBe(true);
      const speedSizes = await speedOptions.evaluateAll((els) =>
        els.map((el) => {
          const rect = el.getBoundingClientRect();
          return { width: rect.width, height: rect.height };
        }),
