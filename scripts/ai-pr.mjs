...
export function githubRepository(url) {
  const match = String(url)
    .trim()
    .match(
      /^(?:https:\/\/github\.com\/|git@github\.com:|ssh:\/\/git@github\.com\/)([A-Za-z0-9][A-Za-z0-9-]*\/[A-Za-z0-9_.-]+?)(?:\.git)?$/,
    );
  requireThat(
    match && !match[1].endsWith('/.') && !match[1].endsWith('/..'),
    'origin must be one credential-free github.com repository URL.',
  );
  return match[1];
}
...
export function parseOptions(argv) {
  ...
  requireThat(
    ['prepare', 'deliver'].includes(action),
    'Use prepare or deliver; branch, base, and remote are fixed.',
  );
  ...
  for (let index = 0; index < args.length; index += 2) {
    ...
    requireThat(
      (key === '--file' || names[key]) && typeof value === 'string' && value.length > 0,
      'Unknown or incomplete delivery option.',
    );
  ...
  if (action === 'prepare') {
    requireThat(
      !result.message &&
        ...
        !result.resume &&
        !result.files.length,
      'prepare accepts only an optional --pr.',
    );
  } else {
    ...
    for (const text of [result.title, result.message].filter(Boolean)) {
      requireThat(
        ...
        'Title/message must be a non-empty single line of at most 200 characters.',
      );
      requireThat(!secret.test(text), 'Credential-like material in delivery metadata.');
    }
    if (result.resume)
      requireThat(
        SHA.test(result.resume) && !result.message && !result.files.length,
        '--resume requires an exact previously verified SHA, without --message or --file.',
      );
    else
      requireThat(
        result.message && result.files.length,
        'A new commit requires --message and explicit --file paths.',
      );
  }
  return result;
}
...
function relativeFile(file, cwd, { body = false } = {}) {
  requireThat(
    typeof file === 'string' && file && !/[\0\r\n\\:*?[\]]/.test(file) && !path.isAbsolute(file),
    'Use exact repository-relative file paths, not globs or pathspecs.',
  );
  requireThat(
    !file.split('/').some((part) => ['', '.', '..'].includes(part)),
    'Path traversal and directory arguments are not allowed.',
  );
  const allowedExample = exampleEnv.test(file) && !blockedPath.test(path.posix.dirname(file));
  requireThat(
    (body && file.startsWith('.artifacts/') && !blockedPath.test(file.slice(11))) ||
      !blockedPath.test(file) ||
      allowedExample,
    'Sensitive or generated paths are excluded from delivery.',
  );
  let current = cwd;
  for (const part of file.split('/')) {
    current = path.join(current, part);
    if (existsSync(current))
      requireThat(!lstatSync(current).isSymbolicLink(), 'Symlink paths require owner review.');
  }
  if (existsSync(current))
    requireThat(lstatSync(current).isFile(), 'Select files, not directories.');
...
export function runDelivery(
  ...
  function command(program, args, { input, soft = false } = {}) {
    ...
    requireThat(
      ...
      program +
        ' ' +
        args[0] +
        ' failed; inspect local/remote state before retrying. No reset or history rewrite was attempted.',
    );
  ...
  if (existsSync(artifacts))
    requireThat(
      ...
      'The artifact directory must not be a symlink.',
    );
  ...
  try {
    ...
    throw new Error(
      "Another delivery may be active. Inspect .artifacts/ai-pr.lock; never clear a live writer's lock.",
    );
  }
  try {
    function api(endpoint, method = 'GET', payload) {
      ...
      return JSON.parse(
        command('gh', args, { input: payload === undefined ? undefined : JSON.stringify(payload) }),
      );
    }
    const urls = git(['remote', 'get-url', '--all', REMOTE]).trim().split('\n');
    const pushUrls = git(['remote', 'get-url', '--push', '--all', REMOTE]).trim().split('\n');
    requireThat(
      urls.length === 1 && pushUrls.length === 1,
      'Multiple origin URLs require owner review.',
    );
    ...
    requireThat(
      info.delete_branch_on_merge !== true,
      'Disable automatic head-branch deletion with owner approval before adopting a persistent ai-changes branch.',
    );
    const currentBranch = () => git(['branch', '--show-current']).trim();
    const sha = (ref = 'HEAD') => git(['rev-parse', '--verify', ref]).trim();
    const cleanIndex = () =>
      requireThat(
        git(['diff', '--cached', '--quiet'], true) !== null,
        'Pre-existing staged changes must remain owner-controlled.',
      );
    const cleanTree = () =>
      requireThat(
    ...
    const remoteHead = () => {
      const value = git(['ls-remote', '--heads', REMOTE, `refs/heads/${HEAD}`])
        .trim()
        .split(/\s+/);
      requireThat(
        value.length === 2 && SHA.test(value[0]) && value[1] === `refs/heads/${HEAD}`,
        'The fixed remote ai-changes branch is missing. Ask the owner; do not recreate it automatically.',
      );
      return value[0];
    };
    for (const name of [
      ...
      requireThat(
        !existsSync(path.resolve(cwd, git(['rev-parse', '--git-path', name]).trim())),
        'An in-progress Git operation must be resolved before delivery.',
      );
    }
    cleanIndex();
    if (options.action === 'prepare' || options.resume) cleanTree();
      ...
      requireThat(
        currentBranch() === HEAD,
        'Delivery is allowed only on ai-changes; run prepare before editing. Never commit on main.',
      );
    ...
    const validatePull = (pr) => {
      requireThat(
        pr.head?.repo?.full_name?.toLowerCase() === repo.toLowerCase() &&
          ...
          pr.head.ref === HEAD &&
          pr.base.ref === BASE,
        'PR repository or head/base mismatch.',
      );
      return pr;
    };
    ...
    requireThat(
      existing ? options.pr === existing.number : options.pr === undefined,
      'An existing PR needs its exact --pr number and related task scope; do not mix unrelated work or create another branch.',
    );
    const push = (commit) => {
      requireThat(
        SHA.test(commit) &&
        ...
        'Branch changed before push; preserve state and inspect the concurrent writer.',
      );
      requireThat(
        git(['merge-base', '--is-ancestor', expectedRemote, commit], true) !== null,
        'Automatic delivery must fast-forward the exact expected remote commit.',
      );
      ...
      requireThat(
        ...
        'Remote or local SHA changed after push; inspect before continuing.',
      );
    };
    ...
    if (options.action === 'prepare') {
      ...
      if (!existing) {
        const ancestor =
          git(['merge-base', '--is-ancestor', expectedRemote, baseCommit], true) !== null;
        if (!ancestor) {
          const merged = pulls('closed').some(
            (pr) => pr.merged_at && pr.head?.sha === expectedRemote && validatePull(pr),
          );
          requireThat(
            merged,
            'ai-changes contains unpublished or unmerged work; do not reuse it for a different task.',
          );
        }
        ...
        requireThat(
          git(['diff', '--quiet', baseCommit, prepared], true) !== null,
          'Idle ai-changes must have the same tree as main before new work.',
        );
        if (prepared !== expectedRemote) push(prepared);
      }
      requireThat(
        currentBranch() === HEAD && sha() === prepared,
        'Local branch changed after prepare.',
      );
      return {
        status: 'prepared',
        repository: repo,
    ...
    if (options.resume) {
      requireThat(
        sha() === options.resume && currentBranch() === HEAD,
        '--resume must match the already verified local ai-changes HEAD.',
      );
      requireThat(
        existsSync(receiptPath) && !lstatSync(receiptPath).isSymbolicLink(),
        'No matching helper delivery receipt; unknown commits cannot be resumed.',
      );
      ...
      requireThat(
        ...
        'Delivery receipt has no task file scope.',
      );
      ...
      const committed = git([
        ...
        .sort();
      requireThat(
        JSON.stringify(recorded) === JSON.stringify(committed),
        'Delivery receipt differs from committed file scope.',
      );
      ...
      requireThat(
        sha() === expectedRemote,
        'Unpublished local commits exist; inspect them instead of bundling them into this task.',
      );
      if (!existing)
        requireThat(
          git(['diff', '--quiet', baseCommit, expectedRemote], true) !== null,
          'Run prepare to synchronize the idle branch before editing.',
        );
      ...
      const staged = git(['diff', '--cached', '--name-only', '--no-renames', '-z'])
        ...
        .sort();
      requireThat(
        JSON.stringify(staged) === JSON.stringify([...files].sort()),
        'Staged scope differs from the explicit task files; inspect without resetting user work.',
      );
      ...
      requireThat(
        sha() === parent,
        'Local branch changed before commit; do not bundle unknown history.',
      );
      ...
      requireThat(
        !existsSync(receiptPath) || !lstatSync(receiptPath).isSymbolicLink(),
        'Delivery receipt must not be a symlink.',
      );
      writeFileSync(
        receiptPath,
    ...
    if (existing) {
      ...
      if (!comment) {
        try {
          comment = api(commentsEndpoint, 'POST', { body: commentBody });
        } catch (error) {
          comment = findEvidence();
      ...
      try {
        ...
        if (
          discovered.length !== 1 ||
          discovered[0].head?.sha !== commit ||
          discovered[0].body !== body
        )
          throw error;
        pr = discovered[0];
      }
    ...
    if (!existing)
      requireThat(
        verified.body === body && verified.title === options.title,
