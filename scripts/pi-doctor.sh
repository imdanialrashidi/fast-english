...
secret_scan_raw="$(mktemp "$doctor_tmp_dir/secrets-raw.XXXXXX")"
...
if find . -maxdepth 6 -type f \
  ...
  xargs -0 -r grep -IEn \
    ...
    >"$secret_scan_raw" 2>/dev/null; then
  if [[ -f scripts/secret-scan-filter.mjs ]]; then
    node scripts/secret-scan-filter.mjs <"$secret_scan_raw" >"$secret_scan_file"
  else
    cp "$secret_scan_raw" "$secret_scan_file"
  fi
