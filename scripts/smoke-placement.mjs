...
const jsonFetch = (path, init) => fetchJson(URL, path, init);
...
async function main() {
  ...
  const { token: activeToken } = await createActiveStudent(URL, suToken);
  ...
  // ==================================================================
  const ov = await createActiveStudent(URL, suToken);
  ...
  const futOv = await createActiveStudent(URL, suToken);
