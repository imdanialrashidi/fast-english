// server/pb_hooks/operator_routes.pb.js
// P1-S2 — Custom routes for the operator payment-review workflow.
//
// Routes:
...
try {
  $app.logger().info("operator_routes: hook file loaded");
} catch (_) {}
...
routerAdd(
  ...
  function (e) {
    ...
    function opCheck(ev) {
      if (!ev || !ev.auth) return { status: 401, body: { code: "unauthorized", message: "Authentication required." } };
      var role = "";
      var acct = "";
      try { role = String(ev.auth.get("role") || ""); acct = String(ev.auth.get("account_status") || ""); } catch (_) {}
      if (acct === "suspended") return { status: 403, body: { code: "operator_access_denied", message: "Access denied." } };
      if (role !== "operator") return { status: 403, body: { code: "operator_access_denied", message: "Access denied." } };
      return null;
    }
    ...
    try {
      ...
      try { $app.logger().error("operator_routes: QUEUE error: " + String(topErr && topErr.message ? topErr.message : topErr)); } catch (_) {}
...
routerAdd(
  ...
  function (e) {
    ...
    try {
      ...
      var reviewerName = "";
      if (reviewerId) { try { var revRec = $app.findRecordById(USERS_COLLECTION, reviewerId); if (revRec) reviewerName = String(revRec.get("name") || ""); } catch (_) {} }
      ...
      try { $app.logger().error("operator_routes: DETAIL error: " + String(topErr && topErr.message ? topErr.message : topErr)); } catch (_) {}
      return e.json(500, { code: "unexpected_error", message: "Internal error." });
    }
  },
  $apis.requireAuth("fep_users")
...
routerAdd(
  ...
  function (e) {
    ...
    try {
      ...
      try { $app.logger().error("operator_routes: RECEIPT error: " + String(topErr && topErr.message ? topErr.message : topErr)); } catch (_) {}
      return e.json(500, { code: "unexpected_error", message: "Internal error." });
    }
  },
  $apis.requireAuth("fep_users")
...
routerAdd(
    ...
    try {
      ...
      try {
        $app.runInTransaction(function (txApp) {
          ...
          var operator = null;
          try { operator = txApp.findRecordById(USERS_COLLECTION, operatorId); } catch (_) {}
          if (!operator) throw new BadRequestError("operator_access_denied", { code: "operator_access_denied" });
          var opRole = String(operator.get("role") || ""); var opStatus = String(operator.get("account_status") || "");
          if (opRole !== "operator" || opStatus === "suspended") throw new BadRequestError("operator_access_denied", { code: "operator_access_denied" });
  ...
  expiresAt.setUTCDate(expiresAt.getUTCDate() + durationDays);
      ...
      if (txError) {
        ...
        var codeMap = { operator_access_denied: 403, request_not_found: 404, request_not_pending: 409, student_suspended: 409 };
        ...
        try { $app.logger().error("operator_routes: APPROVE tx error: " + msg + " raw=" + rawD); } catch (_) {}
        return e.json(500, { code: "unexpected_error", message: "Internal error." });
      }
      ...
      try { $app.logger().error("operator_routes: APPROVE top error: " + String(topErr && topErr.message ? topErr.message : topErr)); } catch (_) {}
      return e.json(500, { code: "unexpected_error", message: "Internal error." });
    }
  },
  $apis.requireAuth("fep_users")
  ...
  function (e) {
    ...
    try {
      ...
      try {
        $app.runInTransaction(function (txApp) {
          var operatorId = String(e.auth.id || "");
          var operator = null; try { operator = txApp.findRecordById(USERS_COLLECTION, operatorId); } catch (_) {}
          if (!operator) throw new BadRequestError("operator_access_denied", { code: "operator_access_denied" });
          var opRole = String(operator.get("role") || ""); var opStatus = String(operator.get("account_status") || "");
          if (opRole !== "operator" || opStatus === "suspended") throw new BadRequestError("operator_access_denied", { code: "operator_access_denied" });
      ...
      if (txError) {
        ...
        var rawD = String(txError.rawData || "");
        var full = msg.toLowerCase() + " " + String(rawD).toLowerCase();
        var codeMap = { operator_access_denied: 403, request_not_found: 404, request_not_pending: 409, approval_conflict: 409 };
        for (var ec in codeMap) { if (full.indexOf(ec) >= 0) { return e.json(codeMap[ec], { code: ec, message: msg }); } }
        try { $app.logger().error("operator_routes: REJECT tx error: " + msg + " raw=" + rawD); } catch (_) {}
        return e.json(500, { code: "unexpected_error", message: "Internal error." });
      }
      ...
      try { $app.logger().error("operator_routes: REJECT top error: " + String(topErr && topErr.message ? topErr.message : topErr)); } catch (_) {}
      return e.json(500, { code: "unexpected_error", message: "Internal error." });
    }
  },
  $apis.requireAuth("fep_users")
